// generated by micro_dart_generator
// ignore_for_file: non_constant_identifier_names

import 'package:micro_dart_runtime/micro_dart_runtime.dart' as m;
import 'package:flutter/src/material/text_button_theme.dart';
import 'package:flutter/foundation.dart';
import 'package:flutter/widgets.dart';
import 'package:flutter/src/material/button_style.dart';
import 'package:flutter/src/material/theme.dart';

const libraryMirror = m.LibraryMirror(
  'package:flutter/src/material/text_button_theme.dart',
  {
    'TextButtonThemeData.': _TextButtonThemeData__$,
    'TextButtonThemeData.lerp': _TextButtonThemeData_lerp$,
    'TextButtonTheme.': _TextButtonTheme__$,
    'TextButtonTheme.of': _TextButtonTheme_of$,
  },
  {},
  {
    'TextButtonThemeData': m.ClassMirror(
      'TextButtonThemeData',
      {
        '#as': TextButtonThemeData_as$,
        '#is': TextButtonThemeData_is$,
        'style': _TextButtonThemeData_style$,
        'hashCode': _TextButtonThemeData_hashCode$,
        '==': _TextButtonThemeData_eq$$,
        'debugFillProperties': _TextButtonThemeData_debugFillProperties$,
      },
      {},
    ),
    'TextButtonTheme': m.ClassMirror(
      'TextButtonTheme',
      {
        '#as': TextButtonTheme_as$,
        '#is': TextButtonTheme_is$,
        'data': _TextButtonTheme_data$,
        'wrap': _TextButtonTheme_wrap$,
        'updateShouldNotify': _TextButtonTheme_updateShouldNotify$,
      },
      {},
    ),
  },
);
Function TextButtonThemeData_as$(
  m.Scope scope$,
  dynamic target$,
) =>
    () => target$ as TextButtonThemeData;
Function TextButtonThemeData_is$(
  m.Scope scope$,
  dynamic target$,
) =>
    () => target$ is TextButtonThemeData;
Function _TextButtonThemeData_style$(
  m.Scope scope$,
  TextButtonThemeData target$,
) =>
    () {
      return target$.style;
    };
Function _TextButtonThemeData_hashCode$(
  m.Scope scope$,
  TextButtonThemeData target$,
) =>
    () {
      return target$.hashCode;
    };
Function _TextButtonThemeData__$(m.Scope scope$) => ({ButtonStyle? style}) {
      return TextButtonThemeData(style: style);
    };
Function _TextButtonThemeData_lerp$(m.Scope scope$) => TextButtonThemeData.lerp;
Function _TextButtonThemeData_eq$$(
  m.Scope scope$,
  TextButtonThemeData target$,
) =>
    (Object other$) => target$ == other$;
Function _TextButtonThemeData_debugFillProperties$(
  m.Scope scope$,
  TextButtonThemeData target$,
) =>
    target$.debugFillProperties;
Function TextButtonTheme_as$(
  m.Scope scope$,
  dynamic target$,
) =>
    () => target$ as TextButtonTheme;
Function TextButtonTheme_is$(
  m.Scope scope$,
  dynamic target$,
) =>
    () => target$ is TextButtonTheme;
Function _TextButtonTheme_data$(
  m.Scope scope$,
  TextButtonTheme target$,
) =>
    () {
      return target$.data;
    };
Function _TextButtonTheme__$(m.Scope scope$) => ({
      Key? key,
      required TextButtonThemeData data,
      required Widget child,
    }) {
      return TextButtonTheme(
        child: child,
        data: data,
        key: key,
      );
    };
Function _TextButtonTheme_of$(m.Scope scope$) => TextButtonTheme.of;
Function _TextButtonTheme_wrap$(
  m.Scope scope$,
  TextButtonTheme target$,
) =>
    target$.wrap;
Function _TextButtonTheme_updateShouldNotify$(
  m.Scope scope$,
  TextButtonTheme target$,
) =>
    target$.updateShouldNotify;
